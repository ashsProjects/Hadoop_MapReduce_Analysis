...
public class Q8 {
    public static class MapperClass extends Mapper<Object, Text, Text, CountAverageTuple> {
        ...
        protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            String artist = columns[38];
            int termsCount = columns[41].split(" ").length;

            double familiarity;
            try {familiarity = Double.parseDouble(columns[32]);}
            catch (NumberFormatException nfe) {familiarity = 0.5;}//set nans to mean

            double hottness;
            try {hottness = Double.parseDouble(columns[33]);}
            catch (NumberFormatException ne) {hottness = 0.3855;}//set nans to mean

            double uniqueness = (0.3333 * familiarity) + (0.3333 * hottness) + (0.003333 * (100 - termsCount));
            outCountAverage.setCount(1);
            outCountAverage.setAverage(uniqueness);
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(config, "q8");
        ...
        if (success) {
            ...
            Job job2 = Job.getInstance(config2, "q8 sort");
            job2.setJarByClass(Q8.class);
